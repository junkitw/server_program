...
static inline void uint256_init_8(BYTE *uint256) {
    if (uint256 == NULL) {
        return;
    }
    for (size_t i = 0; i < 8; uint256[i++] = 0);
}
...
static inline void print_uint256_8(BYTE *uint256) {
    printf ("0x");
...
static inline void uint256_add_8(BYTE *res, BYTE *a, BYTE *b) {
    if (res == NULL || a == NULL || b == NULL) {
        return;
    }
    BYTE aa[8], bb[8];
